...
function renderPicker(selected: 'local-llm-poc' | null = null): string {
...
describe('IntentPicker', () => {
  it('renders all intents as accessible pressed buttons', () => {
    ...
    expect((markup.match(/aria-pressed=/g) ?? []).length).toBe(7);
  ...
  it('marks the selected intent without rendering a second control', () => {
    ...
    expect(markup).toContain('ローカルLLM / AI基盤PoC');
    expect((markup.match(/aria-pressed="true"/g) ?? []).length).toBe(1);
    expect((markup.match(/aria-pressed="false"/g) ?? []).length).toBe(6);
  });
});
