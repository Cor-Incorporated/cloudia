...
2. リネーム候補: `cloudia` または `3d-emotional-chat-ai`（Transfer 時に org 方針で確定。推奨: `cloudia`）。
3. Transfer 実行者は **org admin**（権限がない場合は admin に依頼する Issue をブロッカーとする）。
4. 移管後チェックリスト:
...
## 影響
- 移管完了まで Issue は現行リポに作成し、移管後は自動的に新 URL へ追随（GitHub Transfer の仕様）。
